...
This project, which has been developed, has the main objective of the development of a web application whose features have been extracted from the \textit{Kibana} application in order to create a simplified version of it, but at the same time with a lot of features. To achieve this we are going to use different technologies such as \textit{Elasticsearch}, to store and manage the application's data, and \textit{Angular} as the main front-end technology, with which we are going to implement the user interface and all the application's features.
